...
def load_img(dir, filename): return np.array(Image.open(f"{dir}/{filename}"))


def normalize(input_image, input_mask):
    input_image = tf.cast(input_image, tf.float32) / 255.0
    input_image = tf.image.resize(
        input_image, IMG_SIZE, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def get_image(image_dir, index, df):
    ...
    input_mask = skimage.transform.resize(
        canvas, output_shape=IMG_SIZE+(1,), mode='constant', preserve_range=True)
